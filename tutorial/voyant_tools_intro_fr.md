...
Date: 2018-06-29
Description: "Ce tutoriel francophone d’introduction à Voyant Tools peut être utilisé pour découvrir la plateforme d’une façon autonome ou comme base d’un atelier."
Language: French
Copyright: CC-BY
---
...
Il s’agit simplement de télécharger et décompresser le dossier « VoyantServer\*_\*-M\*.zip » dans le dossier où vous mettez vos applications (prenez la dernière version, quels que soient les chiffres remplacés ici par des astérisques), de lancer l’application en double-cliquant sur le fichier « VoyantServer.jar » (votre ordinateur vous demandera peut-être d’[installer Java](https://www.java.com/fr/download/)) et d’attendre que votre navigateur s’ouvre à une adresse locale (par défaut, http://127.0.0.1:8888/ ; appuyer sur le bouton « Open Web » a le même effet).

Si vous souhaitez explorer un corpus qui vous est familier, il vous suffit de disposer d’un ou de plusieurs fichiers dans un format courant (texte brut, HTML ou XML, voire PDF), ou bien de connaître l’URL d’une page où le texte est accessible sans barrière.
...
* Historique du projet
    ...
    * Dans les années 2000 : HyperPo, logiciel développé par Stéfan Sinclair dans le cadre de ses recherches doctorales sur l’Oulipo
    * À partir de 2008 environ : Voyeur, puis Voyant 1.0 (usage des technologies Flash et Java)
    * Depuis 2015 : Voyant 2.0 (réécriture en HTML5, amélioration des filtres de requête, outils fondés sur le calcul de proximité et les séquences d’éléments, ou n-grammes)
* Principes
    * Ouverture : code en *open source* et ouverture aux contributions de toutes sortes
    * Manipuler, explorer, fouiller les corpus textuels : l’interaction rapide, aisée, voire ludique avec les données est le mot d’ordre, pour apprendre du processus et produire des séries de résultats (de même que les *Text Analysis Recipes* de TAPoR étaient des « recettes »)
...
* `Résumé`/`Summary` offre une syntèse quantitative du corpus.
* `Documents` vous indique la structure du corpus.
* `Syntagmes`/`Phrases` détecte les groupes de mots récurrents.

* `Tendances`/`Trends` met en évidence la distribution du ou des termes sélectionnés au sein du corpus, découpé selon les documents qui le constituent ou selon des segments d’égale longueur.
...
Très accueillante du point de vue des langues et des formats, Voyant Tools est une plateforme d’analyse textuelle et, en tant que telle, résolument logo-centrique et texto-centrique. Vous n’y trouverez pas d’outil pour analyser des images d’imprimés ou de manuscrits, pour aligner textes et images ou pour traiter des flux audio ou vidéo.

### Exemples commentés

Voici quelques exemples, à l’occasion desquels je précise certains points.
...
* Import de fichiers TXT
    ...
    * Latin : César, *La Guerre des Gaules*, texte lemmatisé
        * Sans être parfaite, la lemmatisation suffit pour constater la différence avec le texte précédent. Pour vous en assurer, vous pouvez charger les textes dans deux fenêtres et exporter certaines vues ou listes.
...
Il est temps pour moi de vous laisser expérimenter. N’hésitez pas à partager vos remarques, questions et impressions.
...
Enfin, toutes vos remarques sur ce tutoriel, ou sur l’interface francophone de Voyant, sont bienvenues.

Je vous souhaite de fructueuses lectures, numériques et humaines.

Aurélien Berra, <aurelien.berra@parisnanterre.fr>) et [@aurelberra](https://twitter.com/aurelberra).
